 ...
 *
 * Called once at the page level (global room) so message state outlives tab
 * switches. Messages reset on reload (no persistence — see TECHNICAL_DESIGN
 * open questions).
 */
export function useChatMessages(roomType: RoomType, options?: Options) {
  const active = options?.active ?? true;
  const [messages, setMessages] = useState<ChatMessage[]>(() =>
    roomType === "global" ? SEED_GLOBAL_MESSAGES : []
  );
