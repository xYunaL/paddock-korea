 ...
 */
export function CheerRanking({ ranking, myTeamId }: Props) {
  return (
    <div className="rounded-2xl border border-white/8 bg-[var(--color-charcoal-800)] p-6">
      ...
      <ul className="mt-4 grid gap-1.5">
        {ranking.map((entry, i) => {
          const mine = entry.team.id === myTeamId;
          return (
            <li
              key={entry.team.id}
              className={cn(
            ...
            >
              <span className="w-5 text-center font-mono text-sm text-white/60">
                {i + 1}
              </span>
              <span
